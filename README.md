...
| 公司 / 管理者 | 中国城市+品牌+行业组合（如"北京华盛科技"），约 60% 缩写风格 |
| 创建/修改/打印时间 | 符合时间逻辑的随机时间戳（创建 ≤ 打印 ≤ 修改 ≤ 当前） |
| 修订号 | 1~99 随机整数 |
| 模板路径 | 随机 Linux/macOS/Windows 风格路径 |
| 编辑时长 | 120~2400 分钟（2~40 小时） |
| RSID | 每文档独立的随机十六进制映射 |

**批次唯一性保障（铁律）**：同一批处理的多个文件中，**任何人名和公司名绝不重复**。算法从 6,300+ 组合名和 80+ 独立名构成的 ~6,600 名池中按序分配，数学上保证唯一。

**真实感设计**：
- 名字风格多样：正规写法（"Zhang Wei"）、全大写（"ZHANG WEI"）、姓氏前置（"Zhang, Wei"）、无空格拼接（"ZhangWei"）等
- 公司名以随机大写字母缩写为主（~60%，如 "TX"、"ZRQQ"），更贴近真实使用习惯
- 部分字段随机留空（人名 15%、日期 8%），避免全填满的机械感
- 独立名（"admin"、"user"、"IT"、"ab" 等）不拼接姓氏
