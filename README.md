...
**v1.3 新增可选功能「随机化」**：默认行为仍是清空元数据。勾选复选框后，元数据不会被清空，而是替换为随机的仿真值。适合需要在清除痕迹的同时让文件看起来"正常使用过"的场景。

### 随机化模式（可选功能）

> 默认不勾选 = 清空元数据（原有行为不变）。勾选后才启用随机赋值。
...
| 修订号 | 1~50 随机整数 |
| 模板路径 | Windows/macOS 真实 Office 路径，模板名按文件类型匹配（Word/Excel/PPT） |
...
**抗取证设计**：
- custom.xml 每文件随机填充，打破字节级哈希比对
- ZIP 压缩级别 3-9 随机，打破容器结构指纹
