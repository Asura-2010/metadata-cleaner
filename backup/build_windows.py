...
MSG = {
    ...
    "build_ok": "打包成功！",
    "build_fail": "[错误] 打包失败",
    "output_path": r"dist\MetadataCleaner.exe",
    "output_desc": "Single .exe file, run directly on any Windows PC, no Python needed.",
...
def run_pyinstaller(python_exe):
    """Run PyInstaller to create the executable (single .exe mode)."""
    print(f"\n{MSG['build']}\n")
...
def main():
    ...
    if success:
        output_file = SCRIPT_DIR / "dist" / "MetadataCleaner.exe"
        print()
        print_box([
            MSG["build_ok"],
            "",
